...
class BcutAsrService {
  ...
  Future<List<SubtitleItem>> transcribeAudio(
    ...
    try {
      ...
      final List<int> audioBytes = await _readFileBytes(audioFile, cancelToken);
      _throwIfCancelled(cancelToken);
      ...
      // 3. 分块上传音频
      final etags = await _uploadAudioChunks(audioBytes, uploadAuth, (
        uploaded,
        total,
      ) {
        // 上传进度占 20% - 50%
  ...
  Future<List<String>> _uploadAudioChunks(
    ...
    for (int i = 0; i < clips; i++) {
      ...
      // 稍微延迟一下，避免短时间内过快请求
      await _cancelableDelay(const Duration(milliseconds: 100), cancelToken);
    }
  ...
  Future<String> _commitUpload(
    ...
    if (response.data['code'] != 0) {
      ...
      if (response.data['message']?.contains('异常') ??
          false || response.data['code'] != 0) {
      ...
      throw Exception(
        "提交上传失败: ${response.data['message']} (Code: ${response.data['code']})",
      );
    }
  ...
  Future<String> _createTask(
    ...
    if (response.data['code'] != 0) {
      throw Exception(
        "创建任务失败: ${response.data['message']} (Code: ${response.data['code']})",
      );
    }
  ...
  Future<Map<String, dynamic>> _queryTaskResult(
    ...
    while (retryCount < maxRetries) {
      ...
      await _cancelableDelay(retryDelay, cancelToken);
    }
  ...
  Future<Uint8List> _readFileBytes(File file, CancelToken? cancelToken) async {
