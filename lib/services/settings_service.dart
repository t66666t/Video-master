...
class SettingsService extends ChangeNotifier {
  ...
  static const SubtitleLayoutStyle _legacyGhostLayout =
      SubtitleLayoutStyle(
        fontSize: 28.0,
  ...
  double mediaListItemHeightScale = 0.15;
  double mediaListMainSpacingScale = 0.02;
  double mediaListCrossSpacingScale = 0.03;
    ...
    if (rawJson != null && rawJson.isNotEmpty) {
      try {
        final Map<String, dynamic> decoded =
            Map<String, dynamic>.from(json.decode(rawJson));
        return Alignment(
  ...
  Future<void> _persistAlignmentPreference({
    ...
    await _prefs.setString(
      jsonKey,
      json.encode({'x': align.x, 'y': align.y}),
    );
  ...
  _RegisteredSetting<bool> _boolSetting({
    ...
    return _RegisteredSetting<bool>(
      ...
      read: (prefs, settingKey, fallback) => prefs.getBool(settingKey) ?? fallback,
  ...
  _RegisteredSetting<int> _intSetting({
    ...
    return _RegisteredSetting<int>(
      key: key,
      defaultValue: defaultValue,
      read: (prefs, settingKey, fallback) => prefs.getInt(settingKey) ?? fallback,
  ...
  List<_RegisteredSetting<dynamic>> _createRegisteredSettings() {
    return <_RegisteredSetting<dynamic>>[
      ...
      _boolSetting(
        ...
        defaultValue: true,
        apply: (service, value) =>
            service.enableDoubleTapSubtitleSeek = value,
      ),
      _doubleSetting(
      ...
      _intSetting(
        ...
        defaultValue: 1080,
        apply: (service, value) =>
            service.videoComposeResolutionHeight = value,
      ),
      _boolSetting(
      ...
      _doubleSetting(
        ...
        defaultValue: 0.042,
        normalize: (value) => value.clamp(0.0005, 0.2),
      ...
      _stringSetting(
        ...
        normalize: (value) => value == 'modifiedTime' ? 'modifiedTime' : 'fileName',
  ...
  Future<bool> _updateRegisteredSetting<T>(
    ...
    await setting.updateValue(this, _prefs, value as Object);
    if (notify) {
      notifyListeners();
    }
    return true;
  ...
  Future<void> _migrateLegacyGhostLayoutsIfNeeded() async {
    final int version =
        _prefs.getInt(_ghostLayoutCompatibilityVersionKey) ?? 0;
  ...
  Future<void> savePausePlaybackWhenAppBackgrounded(bool value) async {
    await _updateRegisteredSetting<bool>('pausePlaybackWhenAppBackgrounded', value);
  }
  ...
  Future<void> savePlaybackSpeed(double speed) async {
    await _updateRegisteredSetting<double>('playbackSpeed', speed);
  }
  ...
  Future<PlaybackSpeedLockAction> togglePlaybackSpeedLock(double speed) async {
    ...
    playbackSpeed = speed;
    isPlaybackSpeedLocked = !wasLockedAtSpeed;

    await _prefs.setDouble('playbackSpeed', playbackSpeed);
    await _prefs.setBool('isPlaybackSpeedLocked', isPlaybackSpeedLocked);
    notifyListeners();
