...
class DownloadPartResumeState {
  ...
  bool get hasData => downloadedBytes > 0;
  bool get isComplete => totalBytes != null && totalBytes! > 0 && downloadedBytes >= totalBytes!;
...
class BilibiliDownloadEpisode {
  ...
  String? importedOutputPath; // Output path snapshot for the latest imported download
  List<String> importedVideoIds; // Imported library item ids for the latest imported download
  ...
  bool get hasResumeData =>
      canResume &&
      ((videoResumeState?.hasData ?? false) || (audioResumeState?.hasData ?? false));
  ...
  factory BilibiliDownloadEpisode.fromJson(Map<String, dynamic> json) {
    return BilibiliDownloadEpisode(
      ...
      importedVideoIds: (json['importedVideoIds'] as List?)?.map((e) => e.toString()).toList() ?? [],
